  ...
  for (var _row = 0; _row < inventory_rows; _row++) {
    for (var _column = 0; _column < inventory_columns; _column++) {
      ...
      if (_inventory_index <= array_length(inventory_items) - 1) {
        var _item_pos_x = _pos_x + sprite_w / 2;
        var _item_pos_y = _pos_y + sprite_h / 2;
        draw_sprite(inventory_items[_inventory_index].sprite, 0, _item_pos_x, _item_pos_y);

        // draw counter / text
        var _counter_position_x = _pos_x + sprite_w;
        var _counter_position_y = _pos_y + sprite_h;
        var _counter_radius = sprite_w / 3;
        draw_set(c_black, 1);
        ...
        if (is_between(_mx, _pos_x, _pos_x + sprite_w)) {
          if (is_between(_my, _pos_y, _pos_y + sprite_h)) {
            draw_set(c_gray, 0.1);
            draw_roundrect_ext(
              _pos_x,
              _pos_y,
