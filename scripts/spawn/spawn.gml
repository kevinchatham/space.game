...
function spawn_at_random(_obj, _count, _padding, _layer){
	for (var _i = 0; _i < _count; _i++) {
		var _colliding = true;
	    var _x = irandom_range(obj_space_camera.spawn_port_min_x, obj_space_camera.spawn_port_max_x);
	    var _y = irandom_range(obj_space_camera.spawn_port_min_y, obj_space_camera.spawn_port_max_y);
	    ...
	    while (_colliding) {
			...
			_colliding = inside_view_port(obj_space_camera.camera, _x, _y) || place_meeting(_x, _y, _obj) || too_close(_x, _y, _obj, _padding);
	    }

	    var _new = instance_create_layer(_x, _y, _layer, _obj);

	    if (_new != noone) {
	        show_debug_message("Spawned " + object_get_name(_obj) + " x: " + string(x) + ", y: " + string(y));
	    }
	}
}
...
function safely_despawn(_obj){
	...
	if (outside_spawn_port(obj_space_camera.camera, _obj.x, _obj.y)) {
